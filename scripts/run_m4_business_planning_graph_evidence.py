#!/usr/bin/env python3
"""Generate M4 BusinessPlanningGraph evidence.

Origin: Created by Codex evidence harness worker on 2026-05-17.
Role: Evidence-runner source code only; not acceptance evidence for M4.
...
real GraphDB, real Obsidian vault writes, durable chat/workflow stores, and
AsyncSqliteSaver checkpoints. It stops after the BusinessPlanningGraph approval
resume and records triggered I/W/I, export, and email as not executed.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Aily M4 BusinessPlanningGraph evidence.")
    ...
    return [
        {
            ...
            "selection_reason": "m4_business_planning_graphdb_context_selection",
            "role": "business_planning_context",
        }
...
async def _ingest_foundation(
    *,
    ...
    source = await source_store.store_upload(
        ...
        metadata={
            "intake": "m4_business_planning_graph",
            "origin_path": str(pdf_path),
            "origin_name": pdf_path.name,
            "source_sha256": sha256_file(pdf_path),
            "selection_reason": "M4 representative original PDF for foundation knowledge and business planning",
        },
    )
    ...
    claimed = await source_store.claim_next_source_job(worker_id="m4-business-planning-runner")
    if claimed is None:
        raise RuntimeError("M4 source job was not claimable")

    foundation_run = await workflow_store.create_run(
        workflow_kind="source_foundation",
        input_summary=f"M4 foundation PDF intake: {pdf_path.name}",
        metadata={
            "source_id": source["source_id"],
            "job_id": job["job_id"],
            "job_type": "process_upload_source",
...
async def _run_business_graph(
    *,
    ...
    motive = (
        "Create a confirmed business-planning workflow from the uploaded knowledge foundation. "
    ...
    workflow_run = await workflow_store.create_run(
        ...
        metadata={
            ...
            "pdf_path": str(pdf_path),
            "runner": "m4_business_planning_graph",
        },
    )
    ...
    async with async_sqlite_checkpointer(checkpoint_path) as checkpointer:
        ...
        resumed_result = await graph.ainvoke(
            Command(resume={"approved": True, "decided_by": "m4_business_planning_graph_evidence"}),
            config,
        )
    ...
    selected_context = list((after_plan or before_plan or {}).get("knowledge_context", []))
    selected_node_ids = [
        str(item.get("node_id"))
        for item in selected_context
        if str(item.get("node_id") or "").strip()
    ]
    selected_nodes = await graph_db.get_nodes_by_ids(selected_node_ids)
    return {
        "motive": motive,
...
async def _run() -> int:
    ...
    if not pdf_path.exists():
        raise FileNotFoundError(f"M4 PDF does not exist: {pdf_path}")

    run_id = args.run_id or make_run_id("m4_business_planning_graph")
    ...
    evidence = EvidenceRun(
        ...
        run_id=run_id,
        scenario="m4_business_planning_graph",
        vault_path=vault_path,
        graph_db_path=graph_db_path,
        source_paths=[pdf_path],
        source_selector="explicit_m4_pdf" if args.pdf else "default_pdf_dir_first_pdf",
        source_contexts={
            str(pdf_path): {
                "role": "m4_business_planning_pdf",
                "selection_reason": "M4 representative original PDF for foundation and BusinessPlanningGraph",
            }
        },
        mocked=False,
        real_files=True,
    ...
    try:
        ...
        if before_plan.get("plan_type") != "business_planning":
            failures.append({"check": "workflow_plan_type", "workflow_plan": before_plan})
        ...
        if before_plan.get("workflow_plan_id") != after_plan.get("workflow_plan_id"):
            failures.append({"check": "workflow_plan_id_preserved", "before": before_plan, "after": after_plan})
        ...
        if any(delta != 0 for delta in pre_post_iwi_delta.values()):
            failures.append({"check": "no_iwi_vault_side_effects_before_or_after_business_confirmation", "delta": pre_post_iwi_delta})
        ...
        if not business_payload["selected_context_nodes"]:
            failures.append({"check": "selected_graph_context", "selected_context_nodes": []})

        forbidden_event_prefixes = ("triggered_iwi", "export_", "email_")
        forbidden_events = [
            event
            for event in events
            if str(event.get("type") or "").startswith(forbidden_event_prefixes)
            or str(event.get("type") or "") in {"impact_export_started", "email_send_started"}
        ]
        if forbidden_events:
            failures.append({"check": "no_triggered_iwi_export_email_dispatch", "events": forbidden_events})
        ...
        no_dispatch_status = {
            "triggered_iwi_dispatched": False,
            "export_executed": False,
            "email_executed": False,
            "reason": "M4 runner stops after BusinessPlanningGraph approval resume.",
        ...
        result_summary = {
            ...
            "selected_context_node_count": len(business_payload["selected_context_nodes"]),
        ...
        evidence.write_json("source-record.json", source_record, generation_method="SourceStore.get_source after M4 run")
        evidence.write_json("source-store-jobs.json", source_jobs, generation_method="SourceStore.list_source_jobs after M4 run")
        evidence.write_json("canonical-markdown-package.json", markdown_package, generation_method="SourceStore.get_markdown_package after M4 run")
        ...
        evidence.write_json("business-graph-resumed-result.json", resumed_result, generation_method="BusinessPlanningGraph resumed result via Command(resume={'approved': True})")
        evidence.write_json("chat-records.json", chat_thread, generation_method="ChatStore.get_thread after M4 run")
        ...
        evidence.write_json("workflow-plan-after-confirmation.json", after_plan, generation_method="ChatStore workflow plan after BusinessPlanningGraph resume")
        evidence.write_json("workflow-runs.json", [_run_payload(run) for run in workflow_runs], generation_method="WorkflowRunStore.list_runs after M4 run")
        evidence.write_json("workflow-run-history.json", business_payload["workflow_run_history"], generation_method="WorkflowRunStore.list_run_history for BusinessPlanningGraph")
        evidence.write_json("checkpoint-summary.json", business_payload["checkpoint_summary"], generation_method="SQLite inspection of BusinessPlanningGraph AsyncSqliteSaver checkpoint DB")
        evidence.write_json("pre-post-vault-counts.json", {"pre_business": pre_business_counts, "post_business": post_business_counts, "iwi_delta": pre_post_iwi_delta}, generation_method="Vault counts before and after BusinessPlanningGraph")
        evidence.write_json("selected-context-nodes.json", business_payload["selected_context_nodes"], generation_method="GraphDB selected nodes from BusinessPlanningGraph workflow plan context")
        evidence.write_jsonl("events.jsonl", events, generation_method="M4 captured runtime events")
        ...
        evidence.write_json("no-dispatch-status.json", no_dispatch_status, generation_method="M4 dispatch boundary for triggered I/W/I export email")
        evidence.write_json("result-summary.json", result_summary, generation_method="M4 result summary")
