...
Hosted/private deployment is intentionally fail-closed:

- `HOSTED_MODE=true` or `UI_AUTH_ENABLED=true` requires a non-placeholder
  `UI_AUTH_TOKEN` of at least 16 characters.
- If `HOSTED_MODE=true` and `AILY_DIKIWI_ENABLED=true`, startup requires a real
  provider key via `LLM_API_KEY` or the selected provider-specific key
  (`KIMI_API_KEY`, `DEEPSEEK_API_KEY`, or `ZHIPU_API_KEY`).
- If real DIKIWI is not intended for a smoke deployment, set
  `AILY_DIKIWI_ENABLED=false` explicitly and run the real-provider DIKIWI gate
  separately.
- Keep `.env` out of git. Use `.env.example` as a placeholder-only template.
