<h1 align="center">Aily Copilot for Obsidian</h1>

> This plugin is an Aily fork of
> [`logancyang/obsidian-copilot`](https://github.com/logancyang/obsidian-copilot)
> at upstream commit `bd8829f`. It preserves the upstream React chat surface,
> context controls, chat history, settings, project UI, and AGPL-3.0 license,
> while adding a first-class `Aily` chat mode that routes grounded vault
> questions through the local Aily backend at `http://127.0.0.1:8000`.

## Aily Changes

- Adds `Aily` as a native chat mode in the upstream Copilot mode selector.
- Adds an Aily chain runner that calls `/api/copilot/chat` through Obsidian's
  `requestUrl` API.
- Adds Aily backend settings for base URL, optional bearer token, provider
  routing, and connection checks.
- Keeps upstream Copilot UI behavior instead of replacing it with a separate
  custom pane.
